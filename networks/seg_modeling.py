...
class PIPO_Model(nn.Module):
    def __init__(self, config, img_size=224, self_att=False, cross_att=False, num_classes=None, normalization_sign=True):
        ...
        self.Reconstructor_Q1 = ModalityReconstructor_Q1(in_channels=prompt_channels, base_channels=16)  
        self.Reconstructor_Q2 = ModalityReconstructor_Q2(in_channels=prompt_channels, base_channels=16)  
        self.Reconstructor_Q3 = FeatureReconstructor_Q1(input_dim=256, hidden_dim=512, output_dim=256)   
    ...
    def compute_shared_features(self, mode_Type, X1_share_f_avg, X2_share_f_avg, X3_share_f_avg):
        ...
        for i in range(B):
            ...
            if len(available_feats) > 0:
                ...
                raise ValueError(f"sample {i} no modality")
    ...
    def fuse_modalities_per_sample(self,
        ...
        """
        The three-modal features of each sample are fused to support modality missing.

        Parameter description:
        - mode_Type: shape [B, 3],Each line represents the availability of three modes (1= available,0= missing)
        - share_f_avg: shape [B, C, H, W], The shared features after fusion
        - Xn_share_f_avg, Xn_spec_f_avg: shape [B, C, H, W], The shared/unique features of mode n

        return:
        - X1_fused_ft, X2_fused_ft, X3_fused_ft:The shapes of the fused modal features are all [B, C, H, W].
        """
    ...
    def forward(self, M1, M2, M3, mode_Type='[1,1,1]'):
        ...
        #specific_encoder
        X1_spec_f_avg, X1_spec_f, X1_spec_features = self.transformer1(M1_complete) #The average output of x after different dilated convolution, the avgpooling of the final output x, and the features of each layer
        ...
        spec_f = torch.stack([X1_spec_f, X2_spec_f, X3_spec_f], dim=0) # (3, 24, 256, 1, 1)，This is only used for solving domain classification
